    ...
    <div
        ...
        <div class="">
            <!-- <img class="rounded-full h-10 w-10 float-right shadow-md shadow-gray-400"
                :src="contact.image" /> -->
            <ContactIcon classes="rounded-full h-10 w-10 float-right shadow-md shadow-gray-400 flex justify-center items-center" :contact="contact" stroke="#000" />
            <div class="min-h-[3.5rem]">
                <h3 class="font-bold text-lg text-gray-900">{{ contact.name }}</h3>
                <p class="text-gray-500 text-sm leading-4 pe-10">{{ contact.typecontact.description }}</p>
            </div>
            <p class="text-sm">{{ contact.notes }}</p>
        </div>
        <div class="flex mt-4 absolute bottom-2 w-[calc(100%-1.5rem)]">
            <div class="flex w-full">
                <button title="Editar contacto" @click="$emit('edit', contact)"
                ...
                <button title="Eliminar contacto" @click="$emit('destroy', contact)"
